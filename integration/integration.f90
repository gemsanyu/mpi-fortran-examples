program integration
  ...
  real :: proc_a, proc_b
  real :: z, proc_z
  double precision :: start_time, end_time, duration, max_duration
  ...
  print *, "Process:", process_rank, "A=", proc_a, "B=", proc_b, "duration", duration
  call MPI_REDUCE(proc_z, z, 1, MPI_REAL, MPI_SUM, MASTER_RANK, MPI_COMM_WORLD, ierror)
  call MPI_REDUCE(duration, max_duration, 1, MPI_DOUBLE, MPI_MAX, MASTER_RANK, MPI_COMM_WORLD, ierror)
  call MPI_BARRIER(MPI_COMM_WORLD, ierror)
  if (MASTER_RANK == process_rank) then
    print *,"The result of integrating f(x) from A=:",a,"to B=",b, "is", z
    print *,"Maximum computation time:", max_duration
  end if
